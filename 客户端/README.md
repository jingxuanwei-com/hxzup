...
JVM 启动（-javaagent:launcher-agent.jar）
      ...
      │     ├─ 有 downloads → 多线程下载（最多 parallelDownloads 个）
      │     └─ 无 downloads 或下载失败 → 基准地址/{path} 下载
...
对于清单中的每个文件：
  │
  ├─ 文件有 downloads 列表？
  │   ├─ 是 → 依次尝试 downloads 中的地址（多线程）
  │   │       ├─ 某个地址成功 → 校验 SHA1 → 完成
  │   │       └─ 全部失败 → 回退到服务端下载
  │   └─ 否 → 直接走服务端下载
  │
  └─ 服务端下载：GET {本次连接的服务器地址}/{path}
      ├─ 成功 → 校验 SHA1 → 完成
      └─ 失败 → 进入错误处理 ↓
```

> **服务端地址选择：** 版本检查阶段依次尝试 `servers` 列表，第一个成功响应的地址锁定为本次更新的基准地址，后续所有请求（manifest、文件下载、changelog）均使用该地址。
